...
| `RandomBotTradesMinDelayBetweenTrades` | `uint`, секунды | `1800` | Нижняя граница случайной паузы между трейдами (один трейд за тик на весь инстанс ASF). Диапазон позволяет задавать паузы вплоть до месяцев (напр. 86400–2592000 — от суток до месяца). |
| `RandomBotTradesMaxDelayBetweenTrades` | `uint`, секунды | `7200` | Верхняя граница случайной паузы между трейдами. |
...
> Начиная с этой версии `MinDelayBetweenTrades`/`MaxDelayBetweenTrades` — `uint`, не `ushort` (тот не может представить значения больше ~18 часов, 65535 секунд) — диапазон паузы теперь может доходить до месяцев.
